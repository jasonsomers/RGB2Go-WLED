...
// Applies Matrix2Go's physical 32x8 panel geometry before the first config save.
static void applyMatrix2GoFactoryDefaults()
{
  strip.isMatrix = true;
  strip.panel.clear();
  WS2812FX::Panel panel;
  panel.bottomStart = true;
  panel.rightStart = true;
  panel.vertical = true;
  panel.serpentine = true;
  panel.width = 32;
  panel.height = 8;
  strip.panel.push_back(panel);
...
void WLED::setup()
{
  ...
  #ifdef MATRIX2GO_FACTORY_DEFAULTS
  const bool matrix2goFirstBoot = !WLED_FS.exists(F("/cfg.json"));
  #endif
  bool needsCfgSave = deserializeConfigFromFS();
  #ifdef MATRIX2GO_FACTORY_DEFAULTS
  if (matrix2goFirstBoot) {
    applyMatrix2GoFactoryDefaults();
    needsCfgSave = true;
  }
  #endif
  DEBUG_PRINTF_P(PSTR("heap %u\n"), getFreeHeapSize());
