...
/**
 * Shows the RGB2Go boot logo overlay for a period of time.
 * Clears the screen and prints.
 */
void FourLineDisplayUsermod::overlayLogo(long showHowLong) {
  ...
  // Draw a full-screen 128x64 RGB2Go monochrome splash on 8-row OLEDs.
  // Smaller 128x32 displays fall back to centered text rather than cropping the logo.
  if (u8x8->getRows() >= 8) {
    for (uint8_t tileRow = 0; tileRow < 8; tileRow++) {
      u8x8->drawTile(0, tileRow, 16, const_cast<uint8_t *>(rgb2go_oled_boot_logo_128x64 + (tileRow * 16 * 8)));
    }
  } else {
    u8x8->setFont(u8x8_font_chroma48medium8_r);
    u8x8->draw2x2String(2, 1, "RGB2Go");
  }
  // AI: end
  drawing = false;
